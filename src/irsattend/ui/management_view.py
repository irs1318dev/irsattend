...
class ManagementView(Screen):
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        ...
        self.query_one("#email-qr", Button).disabled = not (student and student['email'])
            ...
            if student:
                ...
                def on_confirm_closed(confirmed: bool | None):
                    if confirmed:
                        ...
                        self.load_student_data()
                        self.query_one("#status-message", Static).update("[green]Student deleted successfully.[/]")
                        self.selected_student_id = None
                        self.query_one("#edit-student", Button).disabled = True
                        self.query_one("#delete-student", Button).disabled = True
                        self.query_one("#selection-indicator", Static).update("No student selected")
                
                await self.app.push_screen(DeleteConfirmDialog(student_name, student['id']), callback=on_confirm_closed)
    ...
    async def action_import_csv(self) -> None:
        def on_import_closed(imported_students: list | None):
            if imported_students:
                ...
                if error_count == 0:
                    self.query_one("#status-message", Static).update(f"[green]Successfully imported {success_count} students.[/]")
                else:
