...
def test_get_events(full_dbase: database.DBase) -> None:
    ...
    # Act
    events = events_mod.Event.get_all(full_dbase)
    # Assert
    assert all(isinstance(evt, events_mod.Event) for evt in events)
...
def test_select_event(full_dbase: database.DBase) -> None:
    ...
    # Arrange
    expected_event = events_mod.Event.get_all(full_dbase)[-1]
    # Act
    event = events_mod.Event.select(
        full_dbase, expected_event.event_date, expected_event.event_type
    )
    # Assert
    assert isinstance(event, events_mod.Event)
...
def test_select_missing_event(full_dbase: database.DBase) -> None:
    ...
    # Act
    event = events_mod.Event.select(
        full_dbase, datetime.date(2019, 12, 31), events_mod.EventType.OUTREACH
    )
    # Assert
...
def test_update_event_description(full_dbase: database.DBase) -> None:
    ...
    # Arrange
    event_to_update = events_mod.Event.get_all(full_dbase)[0]
    key_date = event_to_update.event_date
    key_type = event_to_update.event_type
    assert event_to_update.event_type == events_mod.EventType.MEETING
    assert event_to_update.description is None
    # Act
    event_to_update.update_description(full_dbase, "Test Opportunity")
    updated_event = events_mod.Event.select(full_dbase, key_date, key_type)
    # Assert
...
def test_add_duplicate_event(full_dbase: database.DBase) -> None:
    ...
    # Arrange
    dupe_event = events_mod.Event.get_all(full_dbase)[0]
    # Act, Assert
...
def test_add_new_event(full_dbase: database.DBase) -> None:
    ...
    # Arrange
    new_event = events_mod.Event(
        event_date=datetime.date(2024, 12, 25),
        event_type=events_mod.EventType.OUTREACH,
        description="Christmas Outreach",
    )
    # Act, Assert
    assert new_event.add(full_dbase)  # Returns True if event was added.
    assert (
        events_mod.Event.select(
            full_dbase, datetime.date(2024, 12, 25), events_mod.EventType.OUTREACH
        )
        is not None
    )
...
def test_event_exists(full_dbase: database.DBase) -> None:
    ...
    # Arrange
    existing_event = events_mod.Event.get_all(full_dbase)[0]
    missing_event = events_mod.Event(
        event_date=datetime.date(2025, 1, 1),
        event_type=events_mod.EventType.MEETING,
...
def test_update_event_date(full_dbase: database.DBase) -> None:
    ...
    # Arrange
    event_to_update = events_mod.Event(
        datetime.date(2024, 1, 15),
        events_mod.EventType.NONE,
...
def test_update_event_type(full_dbase: database.DBase) -> None:
    ...
    event_to_update = events_mod.Event.get_all(full_dbase)[0]
    event_to_update.add(full_dbase)
    new_type = events_mod.EventType.COMPETITION
    assert (
        events_mod.Checkin.get_count(full_dbase, event_to_update.event_date, new_type)
        == 0
    )
    # Act
